...
Because friendships are bi directional
...
Percentage correct could be accurate depending on user activity. What's most surprising is degrees of seperation since on social media people tend to have more connections that could reach back to that user.
...
This solution is O(n*a) where n is the number of users and a is the average num friendships. In this solution, as a gets closer to n, the number of collisions that happen when trying to make connections that already exist increases and runtime goes down.