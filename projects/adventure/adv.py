...
def add_relationship(starting_room, direction, ending_room):
...
def closest_room_with_unexplored_exits(starting_room):
    visited = set()
...
while len(room_map) < len(room_graph):
    ...
    for _ in range(len(change_direction)):
        if room_map[room][direction] == '?' and direction in exits:
            ...
            # If already visisted, backtrack and take off traversal path
            if not add_room(next_room):
                player.travel(inverse[direction])
                traversal_path.pop()

            add_relationship(room, direction, next_room)
            break
        else:
    ...
    if not found_unexplored_exit:
        path = closest_room_with_unexplored_exits(room)
